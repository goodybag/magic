...
, run = {
    consumers: function(client, callback){
      ...
      var counter=0;
      mongoDB.consumers.find().skip(startOffset).limit(batchSize).toArray(function(error, consumers) {
        if (error) return onError(error, 'on consumers find for mongo');
        var numOldConsumers=consumers.length;

        var handleNextConsumer = function() {
          console.log('---',++counter,'of',numOldConsumers);
          var oldConsumer = consumers.pop();
          if (!oldConsumer) return callback();

          var upsertConsumer = function() {
            var query;
            if (oldConsumer.barcodeId && oldConsumer.email)
              query = { $or:[{cardId:oldConsumer.barcodeId}, {email:oldConsumer.email}] };
            else if (oldConsumer.barcodeId)
              query = { cardId:oldConsumer.barcodeId };
            else if (oldConsumer.email)
              query = { email:oldConsumer.email };
            else {
              console.log('no email or cardid on', oldConsumer);
              return finishConsumer();
            }

            db.api.users.findOne({ $or:[{cardId:oldConsumer.barcodeId}, {email:oldConsumer.email}] }, function(error, existingUser) {
              if (error) return onError(error, 'on user find for prod');

              // uncomment this to just get a dump of matching users
              // if (existingUser)
              //   console.log(existingUser);
              // return finishConsumer();

              if (existingUser) {
                existingUser.email        = existingUser.email      || oldConsumer.email;
                existingUser.firstName    = existingUser.firstName  || oldConsumer.firstName;
                existingUser.lastName     = existingUser.lastName   || oldConsumer.lastName;
                existingUser.screenName   = existingUser.screenName || oldConsumer.screenName;
                existingUser.cardId       = existingUser.cardId     || oldConsumer.barcodeId;
                existingUser.createdAt    = existingUser.createdAt  || oldConsumer.createdAt;
                // only specify a password if we dont already have one
                existingUser.password     = (!existingUser.password && oldConsumer.password) ? oldConsumer.password : undefined;
                existingUser.passwordSalt = (!existingUser.password && oldConsumer.password) ? '$G00DYBAGR0CK$!' : undefined;

                for (var k2 in existingUser)
                  if (typeof existingUser[k2] == 'undefined')
                    delete existingUser[k2];

                console.log('*', existingUser, oldConsumer);

                db.procedures.updateUser('consumer', existingUser.id, existingUser, function(err, result) {
                  if (error) return onError(error, 'on consumer update to prod');
                  addToGroups(existingUser);
                });
              } else {
                var newConsumer = {
                  email:        oldConsumer.email,
                  password:     oldConsumer.password || undefined,
                  passwordSalt: (oldConsumer.password) ? '$G00DYBAGR0CK$!' : undefined,
                  firstName:    oldConsumer.firstName,
                  lastName:     oldConsumer.lastName,
                  cardId:       oldConsumer.barcodeId,
                  screenName:   oldConsumer.screenName,
                  createdAt:    oldConsumer.createdAt
                };

                for (var k2 in newConsumer)
                  if (typeof newConsumer[k2] == 'undefined')
                    delete newConsumer[k2];

                console.log('#', newConsumer, oldConsumer);

                db.procedures.registerUser('consumer', newConsumer, {dontEncrypt:true}, function(error, result) {
          ...
          var addToGroups = function(consumerRecord) {
            if (oldConsumer.gbAdmin) {
              // add to admin groups
              db.api.usersGroups.insert({ groupId:1, userId:consumerRecord.id }, function(error) {
                if (error) return onError(error, 'on admin group add');
                transferLoyalty(consumerRecord);
              });
            } else
              transferLoyalty(consumerRecord);
          };
          var transferLoyalty = function(consumerRecord) {
            ...
            mongoDB.statistics.find({ consumerId:new mongo.ObjectID(''+oldConsumer._id), 'org.type':'business' }).toArray(function(error, stats) {
              if (error) return onError(error, 'on stats find for mongo');
              // mongoDB.unclaimedbarcodestatistics.find({ barcodeId:oldConsumer.barcodeId, 'org.type':'business' }).toArray(function(error, stats2) {
          ...
          var finishConsumer = handleNextConsumer;
          upsertConsumer();
        };
