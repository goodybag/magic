...
module.exports.get = function(req, res){
  ...
  db.getClient(function(error, client){
    ...
    for (var field in req.fields){
      if (field === "userId" || field === "managerId"){
        as = field;
        name = "id";
      }else{
        as = name = field;
...
module.exports.list = function(req, res){
  ...
  db.getClient(function(error, client){
    ...
    var query = utils.selectAsMap(users, req.fields).from(
      users.join(managers).on(
        users.id.equals(managers.userId)
      )
    );
    ...
    client.query(query.toQuery(), function(error, dataResult){
      ...
      // run count query
      client.query('SELECT COUNT(*) as count FROM users, managers where users.id = managers."userId"', function(error, countResult) {
        if (error) return res.error(errors.internal.DB_FAILURE, error), logger.routes.error(TAGS, error);
  ...
  db.getClient(function(error, client){
    ...
    tx.begin(function() {
      ...
      client.query(query, fields, function(error, result) {
        ...
        client.query(query, [newUser.id, 'manager'], function(err, results){
...
module.exports.del = function(req, res){
  ...
  db.getClient(function(error, client){
    ...
    var query = managers.delete().where(
      managers.id.equals(req.params.id)
    ).toQuery();
...
module.exports.update = function(req, res){
  ...
  db.getClient(function(error, client){
    ...
    tx.begin(function() {
      ...
      query += ' where managers.id = $' + values.push(req.params.id);
