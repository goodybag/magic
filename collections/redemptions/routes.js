  ...
  db.getClient(function(error, client){
    ...
    tx.begin(function(error) {
      ...
      client.query(query, [req.body.tapinStationId], function(error, result) {
        ...
        var query = [
          'SELECT {uls}.*, ({uls}."totalPunches" >= $2) AS "isElite" FROM {uls}',
            'WHERE',
              '{uls}."consumerId" = $1',
              'AND (',
                '(',
                  '{uls}."totalPunches"   >= $2',//$2=punchesRequiredToBecomeElite
        ...
        client.query(query, [req.body.consumerId, bls.punchesRequiredToBecomeElite, elitePunchesReq, regularPunchesReq], function(error, result) {
          if (error) { return res.error(errors.internal.DB_FAILURE, error), tx.abort(), logger.routes.error(TAGS, error); }
