  ...
  it('should update a consumers cardId given that the consumer did not previously have a cardId', function(done){
    ...
    , 'Reward with a punch':
      function(userId, next, finish){
        ...
        var data = { deltaPunches: 4, locationId: locationId };
    ...
    , 'Ensure Punches Are Gooda':
      function(userId, next, finish){
        db.userLoyaltyStats.findOne({ userId: userId, businessId: businessId }, function(error, stats){
          assert(!error);

          assert(stats.numPunches === 4);
          assert(stats.totalPunches === 4);
        });
      }
    ...
    , 'Submit cardupdate request':
      ...
      function(next, finish){
        ...
        tu.post('/v1/consumers/cardupdate', body, function(error, payload, response){
          assert(!error);
    ...
    , 'Consume token':
      ...
      // The website would post to /consumers/cardupdate/:token
      function(token, next, finish){
        tu.post('/v1/consumers/cardupdate/' + token, function(error, payload, response){
          assert(!error);
    ...
    , 'Make sure old user does not exist':
      function(next, finish){
        db.users.findOne(consumer.generatedId, function(error, generatedUser){
          assert(!error);
    ...
    , 'Ensure users cardId has been updated':
      function(next, finish){
        db.users.findOne(consumer.id, function(error, user){
          assert(!error);
