...
describe('GET /v1/consumers/oauth', function() {
  ...
  it('should respond with the correct singly url for authenticating via ' + service1, function(done) {
    tu.get('/v1/consumers/oauth?service=' + service1 + '&redirect_uri=' + callbackUri, function(error, results) {
      assert(!error);
      results = JSON.parse(results);
      assert(!results.error);

      var url = config.singly.apiBaseUrl
      + "/ouath/authorize?client_id="
      + config.singly.clientId
      + "&service="
      + service1
      + "&redirect_uri="
      + callbackUri
      + "/v1/consumers/oauth-callback"
      + "&profile=all"
      + "&account=false"
      ;
  ...
  it('should respond with an invalid service error', function(done) {
    tu.get('/v1/consumers/oauth?service=' + service2 + '&redirect_uri=' + callbackUri, function(error, results) {
      assert(!error);
      results = JSON.parse(results);
      assert(results.error);
