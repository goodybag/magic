...
function createAppServer() {
  ...
  app.get('/name.html', function(req, res){
    ...
    fs.readFile(process.cwd() + '/name.html', 'utf-8', function(error, result){
      if (error) return res.send(404);
      res.send(result);
    });
  });

  app.get('/easyxdm.swf', function(req, res){
    res.header('Content-Type', 'application/x-shockwave-flash');

    fs.readFile(process.cwd() + '/easyxdm.swf', 'utf-8', function(error, result){
      if (error) return res.send(404);
      res.send(result);
    });
  });
