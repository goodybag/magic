...
utils.encryptPassword = function(password, salt, callback){
  ...
  genSalt.call(bcrypt, 10, function(err, salt) {
    if (err) return callback(err);
    bcrypt.hash(password + config.passwordSalt, salt, function(err, hash) {
      if (err) return callback(err);
...
utils.comparePasswords = function(password, encrypted, callback){
  bcrypt.compare(password + config.passwordSalt, encrypted, function(err, success) {
    if (!success)
