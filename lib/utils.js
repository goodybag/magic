...
utils.validate = function(data, schema){
  ...
  for (var field in schema) {
    ...
    v.error = function(msg) {
      var error = { field:field, message:msg };
      errors[field] = ((errors[field]) ? errors[field] : []).concat(error);
    }
    for (var validator in schema[field].validators) {
      var errorMsg = 'TODO'; // errorMessages[validator]
      var validators = v.check(data[field], errorMsg);
      validators[validator].apply(v, [].concat(schema[field].validators[validator]));
    }
  }
