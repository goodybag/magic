...
utils.requireDir = function(path, options, callback){
  var defaults = {
    recursive: false
  ...
  fs.readdir(path, function(error, files){
    ...
    if (!options.recursive){
      for (var i = files.length - 1, name; i >= 0; i--){
        if (files[i].substring(files[i].length - 3) === ".js")
          name = files[i].substring(0, files[i].length - 3);
        else if (files[i].substring(files[i].length - 5) === ".json")
          name = files[i].substring(0, files[i].length - 5);

        modules[name] = require(path + '/' + files[i]);
      }
