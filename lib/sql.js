...
//
// Usage:
//   var withs = new sql.Withs();
//   withs.and('x = 5').and('y = 10').and('(z = 15 OR z = 20)');
//   withs.toString(); // => x = 5 AND y = 10 AND (z = 15 OR z = 20)
function Withs(withs) {
  this.withs = [];
  if (withs) { this.add(withs); }
}
Withs.prototype.add = function(def) {
  this.withs.push(def);
  return this;
};
Withs.prototype.toString = function() {
  if (this.withs.length === 0) { return ''; }
  return 'WITH '+this.withs.join(',\n');
};
...
module.exports = {
  ...
  withs       : function(withs) { return new Withs(withs); },
