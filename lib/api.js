...
Api.prototype.update = function(query, update, callback){
  var defaults = {};
  ...
  db.getClient(function(error, client){
    ...
    var query = sql.query('update "{collection}" {updates} {where} {returning}');
    query.collection = this_.collection;
    query.updates = sql.fields().addUpdateMap(update, query);

    query.where = "";
    for (var key in where){
      if (query.$values.length > 0) query.where += " and ";
      query.where += '"' + key + '" = $' + query.$values.push(where[key]);
    }
