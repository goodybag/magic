...
Api.prototype.findOne = function($query, options, callback){
  ...
  if (options.$join)          query.joins += " " + getJoin('inner',        options.$leftJoin);
  if (options.$innerJoin)     query.joins += " " + getJoin('inner',        options.$leftJoin);
  if (options.$leftJoin)      query.joins += " " + getJoin('left',         options.$leftJoin);
  if (options.$leftOuterJoin) query.joins += " " + getJoin('left outer',   options.$leftJoin);
  if (options.$fullOuterJoin) query.joins += " " + getJoin('full outer',   options.$leftJoin);
  if (options.$crossJoin)     query.joins += " " + getJoin('cross outer',  options.$leftJoin);
