...
//info for bizpanel dashboard
//
//client - database client for easier unit testing (db.getClient)
//       - it's your responsibility to close this client afterwards
//businessId - the id of the business
//callback - called with error/results
...
var exec = function(client, query, callback) {
  var q = query.toQuery();
  client.query(q.text, q.values, function(error, result) {
...
var getTotalUsersForBusiness = function(client, businessId, callback) {
  var query = getUsersQuery();
  query = query.where(businesses.id.equals(businessId))
  exec(client, query, callback);
};

//total likes for a business are comprised of all likes for all products
var getTotalLikesForBusiness = function(client, businessId, callback) {
  var join = businesses
  .join(products).on(products.businessId.equals(businesses.id))
  .join(productLikes).on(productLikes.productId.equals(productLikes.id))
...
var getTotalPunchesForBusiness = function(client, businessId, callback) {
  var query = activity
    .select(activity.star().count().as('totalPunches'))
    .where(activity.type.equals('punch'))
    .and(activity.businessId.equals(businessId))
  var q = query.toQuery();
  return client.query(q.text, q.values, ok(callback, function(results) {
    callback(null, results.rows[0]);
  }));
};

var getTotalLikesForLocation = function(client, locationId, callback) {
  var query = ['SELECT COUNT(*) as "totalLikes"',
    'FROM events',
    'WHERE (events.data::hstore->\'locationId\' = $1::text)'
  ].join(' ');
  return client.query(query, [locationId], ok(callback, function(results) {
    callback(null, results.rows[0]);
  }));
};

var getTotalUsersForLocation = function(client, locationId, callback) {
  var query = getUsersQuery();
  query = query.where(locations.id.equals(locationId))
  exec(client, query, callback);
};

var getTotalPunchesForLocation = function(client, locationId, callback) {
  var query = activity
  .select(activity.star().count().as('totalPunches'))
  .where(activity.type.equals('punch'))
...
var forBusiness = function(client, businessId, callback) {
  var actions = [
    getTotalUsersForBusiness.bind(this, client, businessId),
    getTotalLikesForBusiness.bind(this, client, businessId),
    getTotalPunchesForBusiness.bind(this, client, businessId)
  ];
  runActions(actions, callback);
};

var forLocation = function(client, locationId, callback) {
  var actions = [
    getTotalUsersForLocation.bind(this, client, locationId),
    getTotalLikesForLocation.bind(this, client, locationId),
    getTotalPunchesForLocation.bind(this, client, locationId)
  ];
  runActions(actions, callback);
};
