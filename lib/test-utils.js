//if this is included in a non-test environment I want this to
//throw instead of switching the GB_ENV away from test
if(process.env['GB_ENV'] != "test" && process.env['GB_ENV']) {
  throw new Error("Should not have GB_ENV set but was " + process.env.GB_ENV);
}
...
var hasSetupDatabase = false;
//initialize the test suite
before(function(done) {
  this.timeout(10000);
  //already setup in a higher scope
  if(this.server) {
    return done();
  }
  ...
  this.server.listen(8986, function() {
    if(hasSetupDatabase) return done();
    hasSetupDatabase = true;
    var setupDb = require('../db/setup');
    setupDb(require('../test/db-config.js'), function() {
      exports.loginAsAdmin(function() {
        exports.post('/v1/admin/algorithms/popular', {}, function() {
          exports.post('/v1/admin/algorithms/nearby', {}, function() {
            exports.logout(done);
          });
        });
      });
...
var generatedEmailIndex = 0;

exports.createNewConsumer = function(callback) {
  var email = 'generatedTestConsumer' + generatedEmailIndex++ + '@generated.goodybag.com';
  exports.post('/v1/consumers', {email: email, password: 'password'}, function(error, results, res) {
    try {
...
exports.loginAsTablet = function(callback){
  exports.login({ email: 'tapin_station_0@goodybag.com', password: 'password' }, callback);
};

exports.logout = function(callback){
  exports.del('/v1/session', function(){
    cookie = null;
...
exports.httpRequest = function httpRequest(options, payload, cb) {
  ...
  if (options.headers['User-Agent'] == null && options.headers['user-agent'] == null)
    options.headers['user-agent'] = 'magic-test-utils';

  if (options.path.indexOf(' ') > -1)
    options.path = options.path.replace(/\s/, '%20');
